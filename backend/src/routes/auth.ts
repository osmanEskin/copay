...
auth.post('/register', zValidator('json', registerSchema), async (c) => {
  ...
  return c.json({ token, user: { id: user.id, name: user.name, username: user.username, phone: user.phone, email: user.email } }, 201)
})
...
auth.post('/login', zValidator('json', loginSchema), async (c) => {
  ...
  const token = await sign({ sub: user.id, email: user.email }, JWT_SECRET)
  return c.json({ token, user: { id: user.id, name: user.name, username: user.username, phone: user.phone, email: user.email } })
})
...
auth.patch('/profile', jwt({ secret: JWT_SECRET, alg: 'HS256' }), zValidator('json', updateProfileSchema), async (c) => {
  const payload = c.get('jwtPayload') as { sub: string }
  const { name, username, phone } = c.req.valid('json')

  const [user] = await db.update(users).set({ name, username, phone }).where(eq(users.id, payload.sub)).returning()

  return c.json({ id: user.id, name: user.name, username: user.username, phone: user.phone, email: user.email })
})
