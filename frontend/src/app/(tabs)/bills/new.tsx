...
export default function NewBillScreen() {
  ...
  // Katılımcılar (Basit chip mantığıyla Checkbox simülasyonu)
  const [participants, setParticipants] = useState<string[]>(['Sen', 'Ahmet', 'Mehmet', 'Ayşe']);
  ...
  return (
    <Screen safeArea backgroundColor={colors.background}>
      ...
      <View style={styles.header}>
        ...
        <Text variant="h2" color={colors.text.primary}>Yeni Fatura</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        ...
        <View style={styles.section}>
          ...
          <CurrencyInput 
            label="Tutar (Tahmini veya Kesin)" 
            value={amount} 
        ...
        <View style={styles.section}>
          ...
          <Text variant="body" weight="medium" style={[styles.subTitle, { marginTop: spacing.md }]}>Katılımcılar (Paylaşacaklar)</Text>
          <View style={styles.chipsRow}>
            {payerOptions.map(p => (
              <Chip 
                key={p} 
                label={p} 
                active={participants.includes(p)} 
                onPress={() => toggleParticipant(p)} 
                // Checkbox hissi vermek için aktifse check ikonu gösterilebilir
              />
            ))}
          </View>
